...
public class CompanyUtils extends TableCompanies {
	...
	public static List<Company> getNearestCompanies(List<Company> companies, float lat, float lon)
	{
		...
		{
		    for (Entry<Company, Float> entry : distancesMap.entrySet()) {
	...
	public static List<Company> getSearchedCompanies(int categoryID, String compName, String serveName)
	{
		...
		if (serveName == null){
			if (compName == null){
				...
				companies = CompanyUtils.getSearchedCompanies(compName, categoryID);
			}
		}else {
			...
			if (compName == null){
				...
				List<Company> companies1 = CompanyUtils.getSearchedCompanies(compName, categoryID);
